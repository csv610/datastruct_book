...
Full binary tree with four traversals (inorder, preorder, postorder, level-order), Morris traversal ( extra space), and BST insert/erase/find.
...
\textbf{Why:} You need access to BOTH minimum and maximum simultaneously. \par \textbf{Where to use:} Use for median maintenance, sliding window min/max, and double-ended priority queues. Better than two separate heaps when you need both ends.
...
\textbf{Why:} You need both point lookups AND efficient range queries on sorted data. \par \textbf{Where to use:} Use when your workload combines point lookups with range scans. Leaves are linked for range queries. The standard for database indexes.
...
\textbf{Why:} You need space-efficient full-text indexing with pattern search capability. \par \textbf{Where to use:} Use when memory is a concern: suffix arrays use space vs for suffix trees. Good for full-text search and bioinformatics.
...
\textbf{Why:} You need to efficiently merge sets and check if two elements are in the same set. \par \textbf{Where to use:} Use for Kruskal's MST, connected components in dynamic graphs, cycle detection, and percolation. Nearly per operation.
...
\textbf{Why:} You need predecessor/successor on bounded integer keys. \par \textbf{Where to use:} Use when keys are bounded integers and you need near-constant time operations. Expensive in space: for universe size U.
...
\textbf{Why:} You need to verify data integrity efficiently across distributed systems. \par \textbf{Where to use:} Use for blockchain, distributed databases, Git, and any system requiring tamper-evident data verification. proof per block.
...
Divide-and-conquer: split in half, recursively sort each half, merge. Stable sort with guaranteed .

\textbf{Why:} You need guaranteed and/or stable sort, and can afford extra space. \par \textbf{Where to use:} Use when you need stability (preserving relative order of equal elements), guaranteed , or external sorting (data larger than memory).
...
\textbf{Why:} Your data has many duplicate keys and standard quick sort degrades to . \par \textbf{Where to use:} Use when data has many duplicates: database sorting, histograms, and categorical data. Three-way partition avoids on equal keys.
...
\textbf{Why:} You need the k-th smallest element without fully sorting the array. \par \textbf{Where to use:} Use for finding medians, top-k queries, percentiles, and order statistics. average without modifying the input order.
...
\textbf{Why:} You need a simple in-place sort with better than performance and minimal code. \par \textbf{Where to use:} Use in embedded systems, medium arrays (n ~ 1000), and when code simplicity matters more than optimal performance.
...
\textbf{Why:} You need the simplest possible sort, or want to detect if data is already sorted. \par \textbf{Where to use:} Use for teaching, detecting sorted arrays ( best case), and tiny datasets. Never use in production for large data.
...
\textbf{Why:} Your data consists of integers with a known, small range. \par \textbf{Where to use:} Use when integer keys have small range (k << n log n). beats comparison sorts. Also used as subroutine in radix sort.
...
\textbf{Why:} Your data is uniformly distributed floating-point numbers in [0,1). \par \textbf{Where to use:} Use when data is uniformly distributed: floating-point numbers, histogram equalization. average for uniform input.
...
\textbf{Why:} You need to find the closest pair among n points in 2D/3D. \par \textbf{Where to use:} Use for facility location, collision detection, and clustering. beats the naive for large point sets.
...
Reduces 8 recursive multiplications to 7 at the cost of extra additions. Asymptotically faster than the naive .

\textbf{Why:} You need to multiply large matrices faster than . \par \textbf{Where to use:} Use for large matrix multiplication where n > ~64. Reduces from 8 to 7 recursive multiplications. Practical for large matrices due to cache effects.
...
\textbf{Why:} You need the length of the longest strictly increasing subsequence. \par \textbf{Where to use:} Use for Stock Span, patience sorting, and when you need to find the longest increasing pattern. with patience sorting.
...
\textbf{Why:} You need to find all palindromic substrings in linear time. \par \textbf{Where to use:} Use for longest palindromic substring, palindrome detection in DNA, and text compression. time exploits palindrome symmetry.
...
\textbf{Why:} You need the LCP array from a suffix array. \par \textbf{Where to use:} Use for longest repeated substring, distinct substring counting, and as input to many string algorithms. construction.
...
\textbf{Why:} You need to find pattern occurrences using binary search on a suffix array. \par \textbf{Where to use:} Use for full-text search on suffix arrays. per query. Simpler than suffix tree for static texts.
...
\textbf{Why:} You need shortest paths from a source with non-negative edge weights. \par \textbf{Where to use:} Use for GPS, IP routing, map applications, and robot path planning. log V) with priority queue.
...
\textbf{Why:} You need shortest paths and the graph may have negative edge weights. \par \textbf{Where to use:} Use when negative edges exist: arbitrage detection, currency exchange. Detects negative cycles. time.
...
\textbf{Why:} You need all-pairs shortest paths in a dense graph. \par \textbf{Where to use:} Use for all-pairs shortest paths in dense graphs, transitive closure, and small road networks. simple DP.
...
\textbf{Why:} You need a linear ordering of vertices in a DAG respecting edge constraints. \par \textbf{Where to use:} Use for dependency resolution: build systems, course prerequisites, task scheduling. topological sort.
...
\textbf{Why:} You need shortest paths in a DAG efficiently. \par \textbf{Where to use:} Use for project scheduling, critical path, and DAG optimization. single pass in topological order.
...
\textbf{Why:} You want the simplest augmenting-path max-flow algorithm. \par \textbf{Where to use:} Use for understanding max-flow concepts. Simple but time. Use Dinic's or Edmonds-Karp for better bounds.
...
\textbf{Why:} You want Ford-Fulkerson with guaranteed polynomial time. \par \textbf{Where to use:} Use when you want BFS shortest augmenting path. guaranteed. Simpler than Dinic's for small graphs.
...
\textbf{Why:} You need maximum matching in a bipartite graph. \par \textbf{Where to use:} Use for job assignment, resource allocation, and matching problems. ) via multiple augmenting paths per BFS phase.
...
\textbf{Why:} You need minimum-cost perfect matching in a bipartite graph. \par \textbf{Where to use:} Use for assignment problems: workers to tasks, exam seating, ride-sharing. optimal solution.
...
\textbf{Why:} You need LCS with ) space instead of . \par \textbf{Where to use:} Use for sequence alignment on memory-constrained devices, diff algorithms, and bioinformatics where space is limited.
...
\midrule
sorting.h & Sorting (12 algos) & avg \\
graph.h & Graph (15+ algos) & -- \\
dynamic\_programming.h & DP (4 algos) & -- \\
greedy.h & Greedy (2 algos) & \\
divide\_conquer.h & D\&C (2 algos) & -- \\
backtracking.h & Backtracking (3) & exponential \\
exact\_matching.h & String (5 algos) & \\
approximate\_matching.h & String (4) & \\
multiple\_matching.h & String (2) & \\
string\_compression.h & Compression (4) & \\
dinic.h & Max Flow & \\
flow\_matching.h & Flow/Match (3) & \\
probabilistic.h & Probabilistic (3) & /elem \\
bloom\_filter.h & Probabilistic & /elem \\
avl\_tree.h & Balanced BST & \\
red\_black\_tree.h & Balanced BST & \\
b\_tree.h & Multi-way tree & \\
fenwick\_segment.h & Range query & \\
segment\_tree\_lazy.h & Range query & \\
union\_find.h & Disjoint set & ) \\
kd\_tree.h & Spatial & ) NN \\
bwt.h & BWT/FM-index & \\
nfa.h & Regex & \\
\bottomrule
\end{tabular}
