...
Dynamic array with amortized O(1) append. Grows via reallocation. This is the most fundamental sequential container, analogous to std::vector but built from scratch for pedagogical clarity.
...
Singly linked list using std::unique\_ptr for automatic memory management. Supports forward and reverse iterators.

\textbf{Why:} You need O(1) insert/delete at known positions without shifting elements, and don't need random access. \par \textbf{Where to use:} Use for adjacency lists in sparse graphs, undo/redo buffers, implementing other containers, and when elements are frequently inserted/deleted in the middle.

\textbf{Input:} Elements of any copyable/movable type\\
\textbf{Output:} O(1) insert at front; iteration via range-for
...
\textbf{Why:} You need O(1) insert/delete at both ends AND O(1) delete at arbitrary positions (given iterator). \par \textbf{Where to use:} Use for LRU caches (with hash map), browser history, text editor buffers, and deques. Better than singly linked list when you need backward traversal.
...
\textbf{Class:} dsa::bit\_vector\\

Compact bit storage with bulk operations. Uses native word-level parallelism via std::popcount.
...
\textbf{Class:} dsa::sparse\_matrix\\
...
\textbf{Class:} dsa::sparse\_vector\\
...
FIFO access pattern. Circular queue uses a fixed-size ring buffer for predictable performance.
...
\textbf{Why:} You need O(1) push/pop at both ends AND O(1) random access by index. \par \textbf{Where to use:} Use for sliding window algorithms, BFS with deques, implementing std::deque, and when you need both stack and queue behavior.
...
\textbf{Why:} You need O(1) average-case insert/find/erase by key, and don't need ordering. \par \textbf{Where to use:} Use for symbol tables, caches, frequency counting, database indexes, and any key-value lookup where order doesn't matter.
...
Randomized ordered structure providing balanced BST performance without rotations. Multiple levels of forward pointers give O(log n) expected time.

\textbf{Why:} You need O(log n) ordered operations with simple implementation, and can accept probabilistic balance. \par \textbf{Where to use:} Use as a simpler alternative to balanced BSTs when exact balance isn't critical. Great for concurrent access (lock-free insert), databases, and when you want expected O(log n) without complex rotations.
...
Full binary tree with four traversals (inorder, preorder, postorder, level-order), Morris traversal (O(1) extra space), and BST insert/erase/find.
...
\textbf{Class:} dsa::expression\_tree\\
...
Standard binary max-heap implemented as an array. Also provides heap\_sort() for in-place O(n log n) sorting.

\textbf{Why:} You need fast access to the maximum element with O(log n) insert/delete. \par \textbf{Where to use:} Use as the default priority queue, for heap sort, finding k-th largest, scheduling, and Dijkstra's algorithm. Also useful for median maintenance with two heaps.
...
Generalized d-children heap (default D=4). Higher branching factor improves cache performance for decrease-key operations.

\textbf{Why:} You need a priority queue with many decrease-key operations and better cache performance than binary heap. \par \textbf{Where to use:} Use for Dijkstra's and Prim's algorithms on dense graphs where cache performance matters more than theoretical optimality.
...
\textbf{Why:} You need O(1) access to BOTH minimum and maximum simultaneously. \par \textbf{Where to use:} Use for median maintenance, sliding window min/max, and double-ended priority queues. Better than two separate heaps when you need both ends.
...
\textbf{Class:} dsa::interval\_heap\\
...
\textbf{Class:} dsa::binomial\_heap$<$T$>$\\

Mergeable heap using binomial trees. Efficient meld (merge) operation unlike binary heaps.

\textbf{Why:} You need O(log n) amortized merge of two priority queues, which binary heaps can't do efficiently. \par \textbf{Where to use:} Use for k-way merge in external sorting, Dijkstra's/Prim's on dense graphs, and when you frequently merge two priority queues.
...
\textbf{Class:} dsa::fibonacci\_heap$<$T$>$\\

Theoretically optimal decrease-key performance. Amortized O(1) for insert and decrease-key, O(log n) for delete-min.

\textbf{Why:} You need theoretically optimal O(1) amortized decrease-key for dense graph algorithms. \par \textbf{Where to use:} Use when theoretical optimality matters: Dijkstra's in O(E + V log V), Prim's, and computational geometry. Rarely used in practice due to high constant factors.
...
\textbf{Class:} dsa::pairing\_heap$<$T$>$\\
...
\textbf{Why:} You need O(log n) merge with the leftist property guaranteeing shortest right path. \par \textbf{Where to use:} Use for k-way merge in external sorting, tournament trees, and when you need guaranteed O(log n) merge with simpler code than binomial/Fibonacci heaps.
...
\textbf{Class:} dsa::red\_black\_tree$<$K,V$>$\\
...
\textbf{Why:} You need a general-purpose ordered map with O(log n) all operations and fewer rotations than AVL. \par \textbf{Where to use:} Use as the default ordered map/set. Fewer rotations than AVL on insert/erase make it better for write-heavy workloads. This is what std::map uses.
...
\textbf{Class:} dsa::splay\_tree$<$K,V$>$\\

Self-adjusting BST. Every access splays the element to the root via rotations, exploiting access locality.

\textbf{Why:} Your workload has access locality (recently accessed items are accessed again soon). \par \textbf{Where to use:} Use when access patterns have temporal locality. Splaying brings frequently accessed elements near root, giving amortized O(log n) with good cache behavior.
...
\textbf{Why:} You need a balanced BST with O(log n) split/merge operations for set operations. \par \textbf{Where to use:} Use for implicit treaps (sequence manipulation), order-statistics, split/merge set operations, and when you want expected O(log n) without worst-case balance.
...
\textbf{Class:} dsa::two\_three\_tree$<$K$>$\\
...
\textbf{Class:} dsa::two\_three\_four\_tree$<$K$>$\\
...
\textbf{Class:} dsa::b\_tree$<$K,V,MaxKeys$>$\\

Generalized balanced search tree tuned for disk I/O. Large branching factor minimizes disk reads.

\textbf{Why:} You need a disk-friendly tree that minimizes I/O by maximizing branching factor. \par \textbf{Where to use:} Use for disk-based indexing where each node fits a disk page. Large branching factor (100s of keys per node) minimizes disk reads.
...
\textbf{Class:} dsa::bplus\_tree$<$K,V$>$\\

All data in leaves; leaves linked for efficient range scans. The workhorse of database indexing.

\textbf{Why:} You need both point lookups AND efficient range queries on sorted data. \par \textbf{Where to use:} Use when your workload combines point lookups with range scans. Leaves are linked for O(k + log n) range queries. The standard for database indexes.
...
\textbf{Class:} dsa::suffix\_tree\\

Naive construction for all suffixes of a string. Enables linear-time pattern matching after preprocessing.
...
\textbf{Class:} dsa::suffix\_array\\

Space-efficient alternative to suffix arrays. The sorted array of all suffixes plus LCP (longest common prefix) array.

\textbf{Why:} You need space-efficient full-text indexing with pattern search capability. \par \textbf{Where to use:} Use when memory is a concern: suffix arrays use O(n) space vs O(n$^{2}$) for suffix trees. Good for full-text search and bioinformatics.
...
\textbf{Class:} dsa::interval\_tree\\
...
\textbf{Why:} You need to report all points in an axis-aligned rectangular range. \par \textbf{Where to use:} Use for windowing queries in computational geometry, VLSI checking, and D range reporting. Segment tree on x, sorted by y.
...
\textbf{Why:} You need BOTH range updates (add/set) AND range queries on an array. \par \textbf{Where to use:} Use when you need O(log n) range update AND range query. Lazy propagation defers updates. Essential for competitive programming.
...
\textbf{Why:} You need prefix sums with point updates, and want simpler code than segment trees. \par \textbf{Where to use:} Use for counting inversions, prefix sum queries, and 2D range sums. Simpler than segment trees with similar O(log n) performance.
...
\textbf{Why:} You need to efficiently merge sets and check if two elements are in the same set. \par \textbf{Where to use:} Use for Kruskal's MST, connected components in dynamic graphs, cycle detection, and percolation. Nearly O(1) per operation.
...
\textbf{Why:} You need O(log log U) predecessor/successor on bounded integer keys. \par \textbf{Where to use:} Use when keys are bounded integers and you need near-constant time operations. Expensive in space: O(U) for universe size U.
...
\textbf{Why:} You want vEB-like O(log log U) performance with less space. \par \textbf{Where to use:} Use as a space-efficient alternative to van Emde Boas trees for integer predecessor/successor queries.
...
\textbf{Class:} dsa::merkle\_tree\\

Hash tree for data integrity verification. Each leaf hashes a data block; each internal node hashes its children.

\textbf{Why:} You need to verify data integrity efficiently across distributed systems. \par \textbf{Where to use:} Use for blockchain, distributed databases, Git, and any system requiring tamper-evident data verification. O(log n) proof per block.

\subsubsection{Bloom Filter}
\textbf{Header:} code/include/bloom\_filter.h\\
\textbf{Class:} dsa::bloom\_filter\\
...
\textbf{Why:} O(n$^{2}$) is acceptable for small n, or data is nearly sorted (O(n) best case). \par \textbf{Where to use:} Use for small arrays (n < 20), nearly-sorted data, as the base case in hybrid sorts, and online sorting. Best when data is almost sorted.
...
Divide-and-conquer: split in half, recursively sort each half, merge. Stable sort with guaranteed O(n log n).

\textbf{Why:} You need guaranteed O(n log n) and/or stable sort, and can afford O(n) extra space. \par \textbf{Where to use:} Use when you need stability (preserving relative order of equal elements), guaranteed O(n log n), or external sorting (data larger than memory).
...
\textbf{Why:} You want the fastest average-case sort and can accept O(n$^{2}$) worst case (mitigated by randomization). \par \textbf{Where to use:} Use as the default in-memory sort. Fastest in practice due to cache-friendly partitioning. Randomized pivot makes O(n$^{2}$) extremely unlikely.
...
\textbf{Why:} Your data has many duplicate keys and standard quick sort degrades to O(n$^{2}$). \par \textbf{Where to use:} Use when data has many duplicates: database sorting, histograms, and categorical data. Three-way partition avoids O(n$^{2}$) on equal keys.
...
\textbf{Why:} You need the k-th smallest element without fully sorting the array. \par \textbf{Where to use:} Use for finding medians, top-k queries, percentiles, and order statistics. O(n) average without modifying the input order.
...
\textbf{Why:} You need a simple in-place sort with better than O(n$^{2}$) performance and minimal code. \par \textbf{Where to use:} Use in embedded systems, medium arrays (n ~ 1000), and when code simplicity matters more than optimal performance.
...
\textbf{Why:} Your data consists of integers with a known, small range. \par \textbf{Where to use:} Use when integer keys have small range (k << n log n). O(n + k) beats comparison sorts. Also used as subroutine in radix sort.
...
\textbf{Why:} You're sorting fixed-length integers and want O(d*n) performance. \par \textbf{Where to use:} Use for fixed-length keys: phone numbers, SSNs, dates. O(d*n) where d = number of digits. Faster than comparison sorts for bounded data.
...
\textbf{Why:} Your data is uniformly distributed floating-point numbers in [0,1). \par \textbf{Where to use:} Use when data is uniformly distributed: floating-point numbers, histogram equalization. O(n) average for uniform input.
...
\textbf{Why:} You need to find the closest pair among n points in 2D/3D. \par \textbf{Where to use:} Use for facility location, collision detection, and clustering. O(n log n) beats the naive O(n$^{2}$) for large point sets.
...
Reduces 8 recursive multiplications to 7 at the cost of extra additions. Asymptotically faster than the naive O(n$^3$).

\textbf{Why:} You need to multiply large matrices faster than O(n$^{3}$). \par \textbf{Where to use:} Use for large matrix multiplication where n > ~64. Reduces from 8 to 7 recursive multiplications. Practical for large matrices due to cache effects.
...
\textbf{Why:} You have items with weights/values and a capacity constraint, and items can't be split. \par \textbf{Where to use:} Use for budget optimization, cargo loading, and investment selection. O(nW) pseudo-polynomial time. Use for small W or exact solutions needed.
...
\textbf{Why:} You need the length of the longest strictly increasing subsequence. \par \textbf{Where to use:} Use for Stock Span, patience sorting, and when you need to find the longest increasing pattern. O(n log n) with patience sorting.
...
\textbf{Why:} You need to determine if any subset sums to a target value. \par \textbf{Where to use:} Use for partition problems, fair division, and cryptographic applications. O(n*target) pseudo-polynomial time.
...
\textbf{Class:} dsa::hyperloglog(precision)\\

Probabilistic cardinality estimator. Uses the observation that hash values with many leading zeros are rare.

\textbf{Why:} You need to count distinct elements in a stream with sublinear space. \par \textbf{Where to use:} Use for unique visitor counting, network flow monitoring, and SELECT DISTINCT estimation. O(1) per element, tunable accuracy.
...
\textbf{Class:} dsa::count\_min\_sketch(width, depth)\\
...
\textbf{Why:} You need O(n+m) pattern matching with simple prefix function. \par \textbf{Where to use:} Use as the conceptual stepping stone to KMP. Text pointer never moves backward. Simpler than KMP but skips fewer characters.
...
\textbf{Why:} You need the standard O(n+m) pattern matching algorithm. \par \textbf{Where to use:} Use as the default linear-time pattern matcher. KMP's prefix function skips more characters than Morris-Pratt on mismatch.
...
Uses rolling hash to compare pattern against text windows in O(1). Only verifies on hash match.

\textbf{Why:} You want average-case O(n+m) with simple rolling hash, and can accept O(nm) worst case. \par \textbf{Where to use:} Use for plagiarism detection, multiple pattern search, and when simplicity matters. Rolling hash gives O(1) window comparison.
...
\textbf{Why:} You need optimal O(n+m) matching with skip optimization via critical factorization. \par \textbf{Where to use:} Use for long repeated patterns where skip optimization matters. Theoretically optimal but rarely used in practice.
...
\textbf{Why:} You need to find all palindromic substrings in linear time. \par \textbf{Where to use:} Use for longest palindromic substring, palindrome detection in DNA, and text compression. O(n) time exploits palindrome symmetry.
...
\textbf{Why:} You need the critical index for optimal two-way string matching. \par \textbf{Where to use:} Use as preprocessing for Crochemore-Perrin two-way algorithm. Enables O(n+m) matching with left-to-right text scan.
...
\textbf{Why:} You need to search compressed text without decompressing. \par \textbf{Where to use:} Use for compressed full-text search: genome search, bioinformatics. Supports substring search and counting on compressed data.
...
\textbf{Why:} You need the LCP array from a suffix array. \par \textbf{Where to use:} Use for longest repeated substring, distinct substring counting, and as input to many string algorithms. O(n) construction.
...
\textbf{Why:} You need to find pattern occurrences using binary search on a suffix array. \par \textbf{Where to use:} Use for full-text search on suffix arrays. O(m log n) per query. Simpler than suffix tree for static texts.
...
\textbf{Why:} You need regular expression matching with *, +, ?, |, and (). \par \textbf{Where to use:} Use for regex matching: input validation, log filtering, web scraping. Thompson's NFA construction gives O(nm) matching.
...
\textbf{Why:} You need shortest paths from a source with non-negative edge weights. \par \textbf{Where to use:} Use for GPS, IP routing, map applications, and robot path planning. O((V+E) log V) with priority queue.
...
\textbf{Why:} You need shortest paths and the graph may have negative edge weights. \par \textbf{Where to use:} Use when negative edges exist: arbitrage detection, currency exchange. Detects negative cycles. O(VE) time.
...
\textbf{Why:} You need all-pairs shortest paths in a dense graph. \par \textbf{Where to use:} Use for all-pairs shortest paths in dense graphs, transitive closure, and small road networks. O(V$^{3}$) simple DP.
...
\textbf{Why:} You need a linear ordering of vertices in a DAG respecting edge constraints. \par \textbf{Where to use:} Use for dependency resolution: build systems, course prerequisites, task scheduling. O(V+E) topological sort.
...
\textbf{Why:} You need to know if vertex j is reachable from vertex i. \par \textbf{Where to use:} Use for reachability queries, dependency analysis, and type hierarchy checking. O(V$^{3}$) Boolean matrix.
...
\textbf{Why:} You need shortest paths in a DAG efficiently. \par \textbf{Where to use:} Use for project scheduling, critical path, and DAG optimization. O(V+E) single pass in topological order.
...
\textbf{Function:} dsa::prim(graph, source)\\
...
\textbf{Function:} dsa::kosaraju(graph)\\
...
\textbf{Function:} dsa::tarjan(graph)\\
...
\textbf{Why:} You want the simplest augmenting-path max-flow algorithm. \par \textbf{Where to use:} Use for understanding max-flow concepts. Simple but O(E * max\_flow) time. Use Dinic's or Edmonds-Karp for better bounds.
...
\textbf{Why:} You want Ford-Fulkerson with guaranteed polynomial time. \par \textbf{Where to use:} Use when you want BFS shortest augmenting path. O(VE$^2$) guaranteed. Simpler than Dinic's for small graphs.
...
\textbf{Why:} You need maximum matching in a bipartite graph. \par \textbf{Where to use:} Use for job assignment, resource allocation, and matching problems. O(E sqrt(V)) via multiple augmenting paths per BFS phase.
...
\textbf{Function:} dsa::hungarian(cost\_matrix)\\

Finds minimum-cost perfect matching in a bipartite graph (assignment problem).

\textbf{Why:} You need minimum-cost perfect matching in a bipartite graph. \par \textbf{Where to use:} Use for assignment problems: workers to tasks, exam seating, ride-sharing. O(n$^{3}$) optimal solution.
...
\textbf{Why:} You need LCS with O(min(m,n)) space instead of O(mn). \par \textbf{Where to use:} Use for sequence alignment on memory-constrained devices, diff algorithms, and bioinformatics where space is limited.
...
Finds the shortest string containing all input strings as substrings.

\textbf{Why:} You need the shortest string containing all input strings as substrings. \par \textbf{Where to use:} Use for genome assembly from overlapping fragments and text reconstruction. Greedy approximation.

\begin{lstlisting}
auto scs = dsa::shortest_common_superstring(
    {"ABC", "BCA", "CAB"});
// scs = "ABCAB" (shortest containing all)
\end{lstlisting}
...
\textbf{Why:} You need optimal O(n+m) two-way pattern matching. \par \textbf{Where to use:} Use for optimal string matching with left-to-right text scan. Critical factorization enables O(n+m) with minimal preprocessing.
...
\midrule
sorting.h & Sorting (12 algos) & O(n log n) avg \\
graph.h & Graph (15+ algos) & O(V+E)--O(V$^3$) \\
dynamic\_programming.h & DP (4 algos) & O(nW)--O(mn) \\
greedy.h & Greedy (2 algos) & O(n log n) \\
divide\_conquer.h & D\&C (2 algos) & O(n log n)--O(n$^{2.8}$) \\
backtracking.h & Backtracking (3) & exponential \\
exact\_matching.h & String (5 algos) & O(n+m) \\
approximate\_matching.h & String (4) & O(nm) \\
multiple\_matching.h & String (2) & O(n+m+z) \\
string\_compression.h & Compression (4) & O(n) \\
dinic.h & Max Flow & O(V$^2$ E) \\
flow\_matching.h & Flow/Match (3) & O(VE$^2$) \\
probabilistic.h & Probabilistic (3) & O(1)/elem \\
bloom\_filter.h & Probabilistic & O(k)/elem \\
...
union\_find.h & Disjoint set & O(alpha(n)) \\
kd\_tree.h & Spatial & O(sqrt(n)) NN \\
bwt.h & BWT/FM-index & O(n log n) \\
nfa.h & Regex & O(nm) \\
\bottomrule
\end{tabular}
\end{center}
